...
PROJECT SCOPE (EPHEMERAL NEXUS BUILD)

Darkelf Nexus is the hardened Qt-based edition of the Darkelf-Mini project,
built using Qt WebEngine (Chromium-based rendering).

Nexus represents a coordinated identity and network control layer, designed to
reduce cross-site correlation while maintaining usability and system stability.

CORE DESIGN PRINCIPLES

• Ephemeral by Default
  All browsing data (cookies, cache, history, localStorage, IndexedDB, etc.)
  is stored in memory and discarded when the process exits.

• No Persistent Tracking
  No telemetry, analytics, or external beaconing is included.

• Network Privacy Enforcement
  Requests are filtered and optionally modified through an internal
  interception layer (StealthInterceptor), with optional Tor-based routing
  and controlled circuit rotation.

• Identity Isolation
  Navigation events may trigger controlled identity changes (e.g., network
  circuit rotation and internal seed updates) to reduce cross-site linkability.

• On-Device Threat Detection
  Tracker detection and behavioral analysis are handled by DarkelfMiniAI,
  operating entirely locally without transmitting user data.

• Download Isolation
  Downloads are sandboxed and may be disabled or redirected to ephemeral storage.

──────────────────────────────────────────────────────────────────────────────

SECURITY MODEL

Darkelf Nexus is designed for defense-in-depth:

• HTTPS upgrade enforcement (where possible)
• Request-level filtering (EasyList + heuristic analysis)
• JavaScript execution control
• Optional fingerprint surface reduction and controlled spoofing
• Navigation-based identity rotation (configurable)

Users are encouraged to enable:

• Full-disk encryption (e.g., FileVault)
• OS-level sandboxing and firewall controls

──────────────────────────────────────────────────────────────────────────────

CRYPTOGRAPHY NOTICE

This distribution does not directly implement proprietary cryptographic algorithms.

All encryption (TLS/SSL) is provided by system libraries and Chromium components
within Qt WebEngine.

If you distribute binaries or integrate cryptographic functionality, you are responsible
for compliance with applicable export-control laws.
...
class BootWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # ---- TOR ----
            self.progress.emit(25, "Starting Tor...")
            self.progress.emit(30, "Booting Tor daemon...")
            ensure_tor()
            self.progress.emit(40, "Tor ready")
...
def _find_tor_binary():
    """
    Safely resolve full path to Tor binary.
    Prevents PATH hijacking (Bandit B607).
    """
...
def should_rotate():
    ...
    now = time.time()
    if now - _last_rotation > 10:  # 10 sec cooldown
        _last_rotation = now
        return True
    return False
...
os.environ["QTWEBENGINE_CHROMIUM_FLAGS"] = existing + (
    ...
    # ===== BYPASS TOR FOR YOUTUBE (CRITICAL FIX) =====
...
class EasyListEngine:
    ...
    def should_block(self, url: str, first_party_url: str, req_type: str | None = None) -> bool:
        ...
        # Treat common related domain families as same-site
        RELATED_FAMILIES = (
            ("wikipedia.org", "wikimedia.org", "wmfusercontent.org"),
            ("github.com", "githubusercontent.com", "githubassets.com"),
            ("walmart.com",),
            ("amazon.com", "media-amazon.com", "ssl-images-amazon.com", "images-amazon.com",
            "images-na.ssl-images-amazon.com", "m.media-amazon.com", "a0.awsstatic.com",
            "a1.awsstatic.com", "a2.awsstatic.com", "a3.awsstatic.com", "a4.awsstatic.com",
            "a5.awsstatic.com", "a6.awsstatic.com", "a7.awsstatic.com"),
            ("ebay.com",),
        )
        ...
        # Never block critical same-site resources
        if same_site and req_type in ("script", "xmlhttprequest", "stylesheet", "font", "media", "image"):
            return False

        # Never interfere with common infrastructure and challenge providers
        SAFE_INFRA = (
            "amazonaws.com",
            "cloudfront.net",
            "awswaf.com",
        ...
        # Hard third-party trackers only
        HARD_TRACKERS = (
            "doubleclick.net",
        ...
        if is_third_party and any(req_host == t or req_host.endswith("." + t) for t in HARD_TRACKERS):
            # Keep image requests allowed to avoid storefront/catalog breakage
            if req_type == "image":
                return False
            return True

        # Conservative heuristics only on clearly third-party requests
        if is_third_party and req_type in ("script", "xmlhttprequest", "subdocument", "media"):
            high_signal = (
                "doubleclick",
                "googlesyndication",
                "googleadservices",
        ...
        # Never EasyList-block images to avoid catalog/product breakage
        if req_type not in ("media", "subdocument"):
            return False
...
class StealthInterceptor(QWebEngineUrlRequestInterceptor):
    ...
    def _apply_special_headers(self, info, host: str) -> None:
        try:
            ...
            if is_domain(host, "check.torproject.org"):
                info.setHttpHeader(b"User-Agent", FIREFOX_TOR_UA)

            elif is_domain(host, "youtube.com") or is_domain(host, "youtu.be") \
...
class HardenedWebPage(QWebEnginePage):
    ...
    def inject_tor_letterboxing(self):
        script = r"""
        (() => {
            ...
            if (!window.__darkelf_lb_state) {
                window.__darkelf_lb_state = {};
            }

            const state = window.__darkelf_lb_state;

            const snap = (v, step) => Math.floor(v / step) * step;

            const compute = () => {
                return {
                    width: snap(window.innerWidth, BUCKET_W),
                    height: snap(window.innerHeight, BUCKET_H)
                };
            };
            ...
            const applyPatch = (w, h) => {
                try {
                    safeDefine(window.screen, "width", () => w);
                    safeDefine(window.screen, "height", () => h);
                    safeDefine(window.screen, "availWidth", () => w);
                    safeDefine(window.screen, "availHeight", () => h);

                    safeDefine(window, "innerWidth", () => w);
                    safeDefine(window, "innerHeight", () => h);
            ...
            const drawBars = (w, h) => {
                try {
                    const existing = document.getElementById("__darkelf_lb");
                    if (existing) existing.remove();

                    const overlay = document.createElement("div");
                    overlay.id = "__darkelf_lb";
                    ...
                    const marginX = Math.floor((window.innerWidth - w) / 2);
                    const marginY = Math.floor((window.innerHeight - h) / 2);

                    overlay.style.boxShadow = `
                        inset ${marginX}px 0 0 black,
                        inset -${marginX}px 0 0 black,
                        inset 0 ${marginY}px 0 black,
                        inset 0 -${marginY}px 0 black
                    `;
            ...
            const update = () => {
                try {
                    ...
                    applyPatch(b.width, b.height);
                    drawBars(b.width, b.height);
            ...
            new MutationObserver(() => {
                if (state.width && state.height) {
                    applyPatch(state.width, state.height);
                }
            }).observe(document, { childList: true, subtree: true });

            console.log("[Darkelf] Safe Tor letterboxing:", state.width, state.height);
    ...
    def acceptNavigationRequest(self, url, navtype, isMainFrame):
        ...
        # 🔥 Tor identity rotation on top-level navigation
        if isMainFrame:
            try:
                host = url.host()

                if not hasattr(self, "_last_main_host"):
                    self._last_main_host = None

                # 🔥 Skip rotation for YouTube ecosystem
                YOUTUBE_DOMAINS = (
                    "youtube.com",
                    "youtu.be",
                    "googlevideo.com",
                ...
                def is_youtube(h):
                    return any(h == d or h.endswith("." + d) for d in YOUTUBE_DOMAINS)

                if host and host != self._last_main_host:
                    self._last_main_host = host

                    # 🚫 DO NOT rotate on YouTube
                    if is_youtube(host):
                        print("[Darkelf] ▶️ YouTube detected → skipping Tor rotation")
                    else:
                        if should_rotate():
                            print("[Darkelf] 🌐 New site → rotating Tor identity")
                            tor_new_identity()
...
if __name__ == "__main__":
    ...
    QMenu::item:selected, QMenu::item:hover{background:#00FF9C;color:#181a1b;font-weight:bold;}
    ...
    # ===== SHOW SPLASH FIRST (CRITICAL) =====
    splash = BootSplash()
    splash.show()
    app.processEvents()

    # ===== START TOR (still in main, but AFTER splash shows) =====
    try:
        splash.status.setText("Starting Tor...")
        app.processEvents()

        TOR_PROCESS = ensure_tor()
    ...
    # ===== OPTIONAL: run worker ONLY for heavy background stuff =====
    worker = BootWorker()
    splash._anim = None
    ...
    def boot_done():
        ...
        # CREATE BROWSER ONLY HERE (correct timing)
