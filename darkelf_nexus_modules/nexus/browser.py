...
# --- Nexus Modules ---
from nexus.tor import _tor_running_ok, tor_new_identity
from nexus.miniai import DarkelfMiniAISentinel
...
from nexus.tor import ensure_tor
...
def _darkelf_library_dir():
    """Return the parent Darkelf Library folder without creating it."""
    return os.path.join(os.path.expanduser("~"), "Desktop", "Darkelf Library")


def _ensure_private_dir(path):
    """Create a folder only when needed and restrict it to the current user."""
...
def _darkelf_download_dir():
    """Lazy-created private download temp folder inside Darkelf Library."""
    return _ensure_private_dir(
        os.path.join(_darkelf_library_dir(), "Darkelf Temp Folder")
    )


def _darkelf_snapshot_dir():
    """Lazy-created private snapshot folder inside Darkelf Library."""
    return _ensure_private_dir(
        os.path.join(_darkelf_library_dir(), "Darkelf Snap Folder")
    )
...
class HardenedWebPage(QWebEnginePage):
    ...
    def inject_script(
        self, script_source, injection_point=None, subframes=True, name=None
    ):
        ...
        script_obj.setInjectionPoint(
            injection_point or QWebEngineScript.DocumentCreation
        )
    ...
    def inject_tor_letterboxing(self):
        ...
        self.inject_script(
            script, injection_point=QWebEngineScript.DocumentCreation, subframes=True
        )
    ...
    def inject_iframe_environment_harmonizer(self):
        ...
        """
        self.inject_script(
            js, injection_point=QWebEngineScript.DocumentCreation, subframes=True
        )
    ...
    def acceptNavigationRequest(self, url, navtype, isMainFrame):
        if url.scheme() == "file":
            QMessageBox.warning(
                None, "Navigation blocked", "File URLs are blocked for privacy."
            )
            return False
...
class DarkelfBrowser(QMainWindow):
    ...
    def _make_toolbar(self):
        ...
        self.java_action = QAction(
            make_java_icon(self.accent_color, 18), "JavaScript", self
        )
        self.miniai_action = QAction(
            make_shield_icon(self.accent_color, 18), "MiniAI Monitor", self
        )
    ...
    *{{box-sizing:border-box}}
    ...
    def take_snapshot(self):
        ...
        # Darkelf Library / Darkelf Snap Folder is created only when a snapshot is taken.
        snap_dir = _darkelf_snapshot_dir()
    ...
    def nuke_all_data(self):
        ...
        QMessageBox.information(
            self, "Nuke Complete", "All browser data wiped.\nBrowser will now close."
        )
    ...
    def _handle_download_requested(self, item):
        ...
        # Darkelf Library / Darkelf Temp Folder is created only when a download starts.
        self._download_dir = _darkelf_download_dir()
